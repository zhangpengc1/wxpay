...
import com.rongdu.edu.modules.wxpay.service.WXservice;
...
public class WXserviceImpl implements WXservice {
    ...
    public static final String SPBILL_CREATE_IP = "http://192.168.1.102";
    public static final String NOTIFY_URL = "http://192.168.1.102/v1/weixin/notify.json";
    ...
    public Map<String, String> dounifiedOrder(Map<String,Object> param){
        ...
        data.put("appid", config.getAppID()); // 应用ID 微信开放平台审核通过的应用APPID
        data.put("mch_id", config.getMchID()); // 微信支付分配的商户号
        data.put("nonce_str", WXPayUtil.generateNonceStr()); // 随机字符串，不长于32位
        data.put("body", "订单支付"); // 商品描述
        data.put("out_trade_no",StringUtil.isNull(param.get("outTradeNo"))); // 商户订单号
        data.put("total_fee", StringUtil.isNull(param.get("total_fee"))); // 总金额
        //自己的服务器IP地址
        data.put("spbill_create_ip", "127.0.0.1"); // 自己的服务器IP地址
        //异步通知地址（请注意必须是外网）
        data.put("notify_url", NOTIFY_URL); // 异步通知地址
        //交易类型
        data.put("trade_type", TRADE_TYPE_APP);
        // 附加数据，在查询API和支付通知中原样返回，该字段主要用于商户携带订单的自定义数据
        data.put("attach", StringUtil.isNull(param.get("attach")));
        data.put("sign", md5Util.getSign2(data));
